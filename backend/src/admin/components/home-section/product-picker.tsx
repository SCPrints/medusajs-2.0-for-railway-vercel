...
import { ArrowUpMini, ArrowDownMini, Trash, MagnifyingGlass } from "@medusajs/icons"
...
type Props = {
  /** Ordered list of product handles (the display order on the storefront). */
  value: string[]
  onChange: (next: string[]) => void
}

/**
 * Search the catalog and build an ORDERED list of product handles for a home
 * section. Selected products render as a reorderable list (up/down/remove).
 * Handles that no longer resolve to a product are surfaced with an
 * "unresolved" badge so staff can spot stale curation (e.g. a deleted product
 * or one not yet re-imported) — the storefront silently skips these.
 */
export const HomeSectionProductPicker = ({ value, onChange }: Props) => {
