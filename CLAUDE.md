...
| `GOOGLE_DRIVE_JOBS_FOLDER_ID` | Optional. ID of the Google Drive "Jobs" parent folder (URL part after `/folders/`). Enables the "Create job folder" widget on the admin order detail page — creates `Company \| Customer \| Order#` with `Files` (customer original uploads) + `Mockups` (customizer mockups + revised proofs) subfolders and uploads the order's artwork into them, via the same service account/DWD as SEO analytics (DWD entry needs the `auth/drive` scope + Drive API enabled in GCP). Stamped on `order.metadata.drive_folder` with `uploaded_urls` sync tracking, idempotent; "Sync files to Drive" pushes late-arriving files. | unset — widget hidden |
...
| `IMAGE_AUDIT_ENABLED` | Gates the weekly product-image liveness cron ([backend/src/jobs/audit-product-images.ts](backend/src/jobs/audit-product-images.ts)), which HEAD-checks every thumbnail **and every gallery image** and stamps `metadata.image_audit = { status: "broken", broken_urls, … }` on dead URLs to power the "Broken image" data-quality flag in `/app/product-data`. Off by default because a full sweep hits external supplier CDNs — **ON in production since 2026-06-10**. The on-demand "Scan images" button in the admin works regardless of this gate. | `false` |
| `IMAGE_AUTO_REPAIR_ENABLED` | Gates the weekly supplier-image **self-heal** cron ([backend/src/jobs/repair-supplier-images.ts](backend/src/jobs/repair-supplier-images.ts)) — runs the AS Colour repair + website-scrape scripts and purges the storefront cache, so external CDN rot is fixed without a human in the loop. **ON in production since 2026-06-10.** | `false` |
...
0. **ALL image writes MUST go through `writeProductImages()`** in [backend/src/lib/safe-product-images.ts](backend/src/lib/safe-product-images.ts) — the enforced chokepoint. Do NOT call `productModule.updateProducts({ images })` directly. The guard makes the three failure modes structurally impossible: it rejects any addition that isn't HTTP 200, force-keeps any current image that isn't CONFIRMED dead (404/410), and aborts rather than empty a gallery. Even a `writeProductImages(c, id, [oneUrl])` "replace" call cannot wipe — every existing live image is retained. Pure decision logic is `planImageWrite` ([unit tests](backend/src/lib/__tests__/safe-product-images.spec.ts)); `allowRepairRemovals: true` is the ONLY way to drop images and even then only confirmed-dead ones go.
...
- **Detection/early warning:** the weekly `audit-product-images` cron (gated by `IMAGE_AUDIT_ENABLED`) HEAD-checks every product thumbnail **and gallery image** catalog-wide and flags dead ones (`metadata.image_audit.status = "broken"`, with `broken_urls`), surfaced by the "Broken image" filter in `/app/product-data`. Keep this ON in production so a wiped/dead image is caught automatically, not discovered by a customer. **Self-heal:** the weekly `repair-supplier-images` cron (gated by `IMAGE_AUTO_REPAIR_ENABLED`) then runs the AS Colour repair + scrape + cache purge automatically — detection AND repair are closed-loop as of 2026-06-10.
- **Image array order is NOT a contract.** Selection of which image to display (customizer canvas front/back, gallery hero, cart thumbnails) MUST be token/colour-aware (`pickFrontishGarmentUrl` / `garmentUrlViewRank` in [storefront/src/modules/products/lib/variant-options.ts](storefront/src/modules/products/lib/variant-options.ts)), never "first match in `product.images` order" — repair/scrape scripts and re-imports append and reorder freely. The 2026-06-10 incident: order-dependent front selection silently flipped to BACK photos for every colour the repair touched.
...
| `0 17 * * 0` | Mon 03:00 | [audit-product-images.ts](backend/src/jobs/audit-product-images.ts) | HEAD-check every product thumbnail + gallery image; flag dead URLs as `metadata.image_audit.status = "broken"` (gated by `IMAGE_AUDIT_ENABLED`) |
| `0 19 * * 0` | Mon 05:00 | [repair-supplier-images.ts](backend/src/jobs/repair-supplier-images.ts) | Self-heal supplier CDN rot: AS Colour repair + website scrape + storefront cache purge (gated by `IMAGE_AUTO_REPAIR_ENABLED`) |
...
**Scope (v2, 2026-06-10)**: checks the thumbnail **and every gallery image** — per-colour CDN rot leaves the thumbnail healthy while that colour's PDP gallery + customizer canvas break, so thumbnail-only auditing misses the failure mode that actually bites. Stamps `broken_count` + `broken_urls` (capped at 20) alongside `status`. Emits a `image_audit_completed` PostHog event per run.

**Self-heal cron** ([backend/src/jobs/repair-supplier-images.ts](backend/src/jobs/repair-supplier-images.ts), `0 19 * * 0`, gated by `IMAGE_AUTO_REPAIR_ENABLED`): runs `repair-ascolour-images` (removes confirmed-dead URLs, recovers live size-variants) + `scrape-ascolour-website-images` (append-only fill from ascolour.com) and purges the storefront product cache — so supplier CDN rot is repaired automatically every week instead of by a human after a customer finds it. Both scripts are idempotent + skip-healthy; all writes go through the `writeProductImages` guard.
...
**Per-product technique restriction** (`metadata.print_methods`): a lightweight override layered on top of the assigned profile — `["print"]` = print-only garment, `["embroidery"]` = embroidery-only, absent/both = defer to the profile. At resolve time each area's `methods` is intersected with it and emptied areas drop (e.g. a print-only neck tag disappears on an embroidery-only garment). Set via the **Techniques** tickboxes on the product-detail widget (profile mode); `applyMethodFilter` / `sanitizeMethodFilter` apply it in both the backend resolved-preview ([print-profile route](backend/src/api/admin/products/[id]/print-profile/route.ts)) and the storefront resolver. This is the "this garment is embroidery-only" control — distinct from editing a shared profile (which changes every product on it) or going full-custom (which re-defines locations).
...
**Seeded system profiles** (`is_system: true` — editable but not deletable): `short-sleeve-garment`, `long-sleeve-garment`, `sleeveless`, `cap-headwear`, `beanie`, `bag-tote`, `puffer-jacket`, `socks` (print-only, front+back, A6). These reproduce the previous hard-coded heuristics as explicit data (e.g. puffer = embroidery-only, beanie = embroidery-only front, sleeves A6 on short-sleeve / A3 on long-sleeve). **Per-area `methods[]`** is how print-vs-embroidery is controlled — set each area's Techniques (Print (DTF) / Embroidery / both) in the profile editor; the customizer's method picker reads `profileMethodsForSide` and enforces it (embroidery-only areas lock the side to embroidery).
