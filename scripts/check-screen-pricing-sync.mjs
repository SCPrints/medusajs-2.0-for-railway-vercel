#!/usr/bin/env node
/**
 * Verifies the storefront mirror of `scp-screen-print-pricing.ts` agrees with
...
const BACKEND_PATH = resolve(repoRoot, "backend/src/lib/scp-screen-print-pricing.ts")
const STOREFRONT_PATH = resolve(
  repoRoot,
  "storefront/src/modules/customizer/lib/scp-screen-print-pricing.ts"
)
...
try {
  ...
  console.error(`✗ screen-pricing sync check: cannot read file — ${err.message}`)
  process.exit(1)
}

if (backend !== storefront) {
  console.error(
    "✗ scp-screen-print-pricing.ts files are OUT OF SYNC.\n" +
...
console.log("✓ scp-screen-print-pricing.ts files are in sync")
